  ...
  <div class="orderList">
    <div class="search">
      <Row class="searchLine">
        <i-col span="6" class="orderLine"  v-if="selectArr.indexOf(activeDate)>-1?item.checkStatus=='1'?'验收中':item.checkStatus=='2'? '已驳回':item.checkStatus=='3'?'已完成':'未执行':'未执行'">
      ...
      <Row class="searchLine">
        ...
        <i-col span="6" class="orderLine">
          <div>供应商名称：</div>
          <Input v-model="orderInfo.shopName" clearable placeholder="请输入供应商名称" style="width: 250px" />
        </i-col>
      ...
      <div class="orderItem" v-for="(item, index) in records" :key="index">
        <div class="orderItem_orderHeader">
          <div class="orderNumber">
            订单编号：{{item.systemOrderNo}}
            <span
              style="color:blue;cursor: pointer;"
              @click="handleCopy(item.systemOrderNo,$event)"
            >复制</span>
          </div>
          ...
          <div class="orderType">支付日期：{{item.payTime}}</div>
           <div class="orderType">支付方式：{{item.payWay}}</div>
            <div class="orderType">总价格（元）：{{item.totalGoodsAmout}}</div>
            <div class="orderType">运费（元）：{{item.totalCarriage}}</div>
           <div class="orderType">实收总价（元）：{{item.totalCarriage + item.totalGoodsAmout }}</div>
        </div>
        <div class="orderMess" v-for="(orderList_child, index1) in item.shopList" :key="index1">
          <Row v-if="index1===0">
            ...
            <i-col span="5" class="orderLine orderNum">
              <div>
                子订单号：{{orderList_child.orderNo}}
                <span
                  style="color:blue;cursor: pointer;"
                  @click="handleCopy(orderList_child.orderNo,$event)"
                >复制</span>
              </div>
            </i-col>
            <i-col span="3" class="orderLine ordertype">
              <div class="type">
                子订单状态：
                <span
                  :class="getClass1(orderList_child.orderStatus)"
                >{{orderList_child.orderStatus }}</span>
              </div>
            </i-col>
            <!-- "21212121"{{orderList_child.clearingInfo}} -->
            <i-col span="4" v-if="item.orderType==='拼团' && orderList_child.orderStatus!=='已取消'" class="orderLine orderItem-type">
              主单拼团状态：
              <div
                class="type"
              >{{Number(orderList_child.grouponStatus )===0? '拼团中' : Number(orderList_child.grouponStatus )===1?'拼团成功' :'拼团失败'}}</div>
            </i-col>
            <i-col span="2">
              <i-button
                ...
                type="primary"
                @click="order(orderList_child,item)"
                class="btn"
              >普通发货</i-button>
            </i-col>
            <i-col span="2">
              <i-button
                v-if="('待发货'===orderList_child.orderStatus && item.orderType==='拼团' && Number(orderList_child.grouponStatus )===1) ||　('待发货'===orderList_child.orderStatus&& item.orderType!=='拼团')"
                type="primary"
                @click="orderPart(orderList_child,orderList_child.orderNo)"
                class="btn"
              >分包裹发货</i-button>
            </i-col>
            <i-col span="2">
              <i-button class="btn" type="primary" @click="JumpDetail(orderList_child.orderNo)">查看详情</i-button>
            </i-col>
          </Row>
          <Row v-if="index1!==0">
            <div v-if="item.orderType==='普通订单'">
              <i-col span="4" class="orderLine message">
                <div>店铺信息：{{orderList_child.shopName}}</div>
              </i-col>
              ...
              <i-col span="5" class="orderLine ordertype">
                <div class="type">
                  子订单状态：
                  <span
                    :class="getClass1(orderList_child.orderStatus)"
              ...
              <i-col span="2" class="orderLine ordertype">
                <i-button type="primary" v-if="'待发货'===orderList_child.orderStatus"  @click="order(orderList_child)" class="btn">普通发货</i-button>
              </i-col>
                <i-col span="2" class="orderLine ordertype">
                  <i-button
                v-if="('待发货'===orderList_child.orderStatus && item.orderType==='拼团' && Number(orderList_child.grouponStatus )===1) ||　('待发货'===orderList_child.orderStatus&& item.orderType!=='拼团')"
            ...
            <div v-else>
              <i-col span="21" class="orderLine orderItem-type-reserve">
                <div
                  class="type"
                  v-if="item.orderType==='拼团'"
                >{{Number(orderList_child.grouponStatus )===0? '拼团中' : Number(orderList_child.grouponStatus )===1?'拼团成功' :'拼团失败'}}</div>拼团状态：
              </i-col>
              <i-col span="3"></i-col>
            </div>
          </Row>
            ...
            <div class="orderDetail-orderItem">
              <div class="orderDetail-orderMess">
                <Row class="header">
                  ...
                  <i-col :span="getSpanNum(item.orderType,orderList_child)" class="orderLine ordertime">
                    <div>实收价格（元）</div>
                  </i-col>
                  <i-col  v-if="item.orderType==='拼团' &&orderList_child.orderStatus!=='已取消'"  span="2" class="orderLine ordertime">
                    <div>拼团状态</div>
                  </i-col>
                  <!-- <i-col
                ...
                <Row
                  class="ListDetail"
                  v-for="(item1, index1) in orderList_child.orderList"
                  :key="index1"
                >
                  <i-col
                    span="6"
                  ...
                  <i-col span="3" class="orderLine ordertime">
                    <div>{{item1.goodsAmount}}</div>
                  </i-col>
                  <i-col :span="getSpanNum(item.orderType,orderList_child)" class="orderLine ordertime">
                    <div>{{item1.practicalClearing}}</div>
                  </i-col>

                  <i-col
                    v-if="item.orderType==='拼团'&&orderList_child.orderStatus!=='已取消'"
                    span="2"
                    class="orderLine ordertime"
                    style="color:red"
                  >
                    <div>{{Number(orderList_child.grouponStatus )===0? '拼团中':(item1.clearingInfo===''? (Number(orderList_child.grouponStatus )===1?'拼中' :'拼不中') : (Number(item1.clearingInfo )===0? '拼不中' :'拼中') )}}</div>
                  </i-col>
                  <!-- <i-col
                    :span="getSpanNum(item.orderType)"
                    class="orderLine ordertime"
    ...
    <Modal title="商品分包裹发货" v-model="modal11 " class-name="vertical-center-modal" @on-ok="ModalOK2" @on-cancel="clearTrackingList">
      <div class="item">
...
export default {
  ...
  methods: {
    ...
    orderPart (val, e) {
      console.log(e)
      this.modal11 = true;
      this.systemOrderNo = Number(e)
      this.$get('/orderMaster/pull?orderNo=' + Number(e)).then(res => {
        // console.log(res)
        this.goodsList = res
      })
    },
    ...
    exportExel () {
      let baseUrl = 'http://admin-beta.upmall.cntracechain.com/admin/orderMaster/export'
