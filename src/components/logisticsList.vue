<template>
  <div class="div-bg bg-white">
    ...
    <div style="padding-bottom: 0.5rem;">
      <div
        class="bg-white"
        style="width: 92%; margin-left: 4%;margin: auto;padding-left: 15px;padding-right: 15px;display:flex"
      >
        <div style="font-size: 1.2rem;color: #111111;width:110px">
          物流跟踪
          <!--物流跟踪-->
        </div>
        <div style="flex:1">
...
export default {
  ...
  methods: {},
  created() {
    this.logisticsList = this.TrackInfo.trackList;
  },
  watch: {
    TrackInfo: {
      handler() {
        ...
        if (this.TrackInfo) {
          ...
          this.TrackInfo.trackList.forEach(element => {
            let obj = {
              message: element.context,
              messageDate: element.ftime
            };
